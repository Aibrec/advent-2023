...
def rank_hand(hand):
    ...
    if 5 in count_to_card:
        ...
        if jokers_available == 2:
            return 7
        elif jokers_available == 1:
            return 6  # Best option is 4 of a kind
        else:
            return 4
    elif 2 in count_to_card and len(count_to_card[2]) == 2:
        if jokers_available == 1:  # Can't have more than 1 joker, using 4 cards
            return 5  # full house on either pair
        else:
            return 3
    elif 2 in count_to_card:
        if jokers_available == 3:
            return 7
        elif jokers_available == 2:
            # 4 of a kind beats full house
            return 6
        elif jokers_available == 1:
            # One pair and two other cards, plus a joker
            # best option is three of a kind
            return 4
        else:
            return 2
    else:
        if jokers_available == 5:
            return 7
        elif jokers_available == 4:
            return 7
        elif jokers_available == 3:
            # 4 of a kind beats full house
            return 6
        elif jokers_available == 2:
            # three of a kind
