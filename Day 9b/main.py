...
with open(file_path, 'r') as file:
    ...
    def extrapolate_history_backward(history):
        for i in range(len(history)-2, -1, -1):
            # Last value adds the value in the row below it
            first_value_from_row_below = history[i+1][0]
            first_in_current = history[i][0]
            history[i].insert(0, (first_in_current - first_value_from_row_below))
    ...
    for history in histories:
        extrapolate_history_backward(history)
        sum_of_new_values += history[0][0]
        for line in history:
